...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var alumnos = [
        Alumno(nombre: "Cristina", calificacion: 100)
    ]
    ...
    @IBAction func agregarNuevoBtn(_ sender: UIBarButtonItem) {
        ...
        alerta.addTextFiel { calificacionAlerta in
            calificacionAlerta.placeholder = "Ingresa la calificacion"
            calificacionAlerta.keyboardType = .numero
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "enviarNombre"{
            let objDestino = segue.description as! VistaDetalladaViewController
            objDestino.recibirNombre = alumoSeleccionado
        }
    }
